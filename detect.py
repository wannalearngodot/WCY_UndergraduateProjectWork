...
sys.path.append(ROOT_DIR)  # To find local version of the library,把项目的路径加入系统的库（import的包）路径
...
for i in range(0, count):
    ...
    print(image_id)
    #read image
    image = skimage.io.imread(img_floder+'/'+imglist[i])
    cv2_image=cv2.imread(img_floder+'/'+imglist[i])
    ...
    r = results[0]
    #这一步是画mask和写坐标的关键
    display_instances(imglist[i], image, r['rois'], r['masks'], r['class_ids'], class_names, r['scores'])
    #visualize.display_instances(imglist[i], image, None, r['masks'], r['class_ids'], class_names, r['scores'])
    #visualize.display_differences(imglist[i], image,gt_bbox, gt_class_id, gt_mask, r['rois'], r['class_ids'], r['scores'], r['masks'],class_names, title="", ax=None,show_mask=True, show_box=True,iou_threshold=0.5, score_threshold=0.5)